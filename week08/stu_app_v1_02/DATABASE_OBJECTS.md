...
> 本文档配合 `week08/stu_app_v1_02` 使用，面向课堂教学场景，系统讲解如何在真实 CLI 项目中引入**视图（View）、触发器（Trigger）、存储过程（Stored Procedure）、函数（Function）以及完整性约束（Integrity Constraint）**等数据库对象。
...
| `trg_after_student_major_change_log` | `AFTER UPDATE` | 【课后作业】学生专业变更后，记录变更日志 |
...
该脚本会重建 `students`、`course`、`sc`、`teacher` 四张基表，并插入 4 名学生、6 位教师、6 门课程、14 条选课记录及对应成绩。
...
5. 保存文件后，运行 `python main.py` → 选择 **23. 视图管理** → 选择 **6. 【课后作业】创建学生学分统计视图**，验证是否创建成功。
...
4. 保存文件后，运行 `python main.py` → 选择 **24. 触发器管理** → 选择 **6. 【课后作业】创建专业变更日志触发器**。
...
4. 保存文件后，运行 `python main.py` → 选择 **25. 存储过程管理** → 选择 **7. 【课后作业】创建并调用成绩加分存储过程**。
5. 验证：选择课程号和加分值后，回到主菜单选择 **18. 查看所有选课记录**，观察该课程的成绩是否正确变化（且不超过 100）。
...
4. 保存文件后，运行 `python main.py` → 选择 **26. 函数管理** → 选择 **6. 【课后作业】创建 GPA 转换函数**。
...
3. 保存文件后，运行 `python main.py` → 选择 **27. 完整性约束管理** → 选择 **4. 【课后作业】添加手机号格式 CHECK 约束**。
