...
class Database:
    ...
    def update_course(self, course_id, update_data):
        ...
        query = """
        UPDATE course
        SET course_name=%s, teacher_id=%s, credit=%s
        WHERE course_id=%s
        """
        try:
            self.cursor.execute(query, (
                update_data['course_name'], update_data['teacher_id'],
    ...
    def get_student_courses(self, student_id):
        ...
        query = """
        SELECT sc.*, c.course_name, t.name as teacher_name, c.credit
        FROM sc
    ...
    def get_all_course_selections(self):
        """获取所有选课记录（关联学生和课程信息）"""
        query = """
        SELECT sc.*, s.name as student_name, c.course_name, t.name as teacher_name, c.credit
        FROM sc
